...
$(function(){//jquery的加载事件
	$('#table').bootstrapTable({
		  url: '${pageContext.request.contextPath}/inpatient/queryInpatientDepartment.action',//访问后台（json）的地址
		  ...
		  },
		  {
		    ...
		    title: '所在科室'
			  ...
			  formatter:function(value,row,index){
				  var updateButton = "<button  onclick=\"myUpdateClick("+row.hospitalizationid+")\" type=\"button\" class=\"btn btn-link\"><span class=\"glyphicon glyphicon-edit\" style=\"color: rgb(0, 0, 60); font-size: 17px;\"> </span></button>";	
				  var deleteButton = "<button onclick=\"myDeleteClick("+row.hospitalizationid+")\" type=\"button\" class=\"btn btn-link\"><span class=\"glyphicon glyphicon-trash\" style=\"color: rgb(0, 0, 60); font-size: 17px;\"> </span></button>";
	...
	$.ajax({
		type:'post',//提交方式
		url:'${pageContext.request.contextPath}/bed/queryBed.action',//访问后台的地址   查询所有的病人信息
		async:false,//  true：异步     false:同步
		dataType: "json",//数据类型
		success: function(data){
			...
			if(data != null){
				var optionhtml = '';//定义一个字符串变量	
				for(var i=0;i<data.length;i++){			
					optionhtml +='<option value="'+data[i].bednumber+'">'+data[i].bednumber+'</option>';			
				}		
				$("#myUpdateBed").html(optionhtml);
				$("#myAddBed").html(optionhtml);
			} 
		}
	})
 ...
 <!--修改-->
  function myUpdateClick(hospitalizationid){
  	
  	$.ajax({
  		 type:"post",//请求方式
  		 url:"${pageContext.request.contextPath}/inpatient/updateInpatient.action",//根据编号查询用户的信息
  		 data:{hid:hospitalizationid},//传参数到后台
  		 ...
  		 success:function(data){
  			 ...
  			 //data：指后台查询后返回来的用户信息
  			     $('#myUpdateModal').modal().on('shown.bs.modal',
  					    function() {
  			    	 //给隐藏框赋值
			    	 $("#myUpdatehospitalizationid").val(data.hospitalizationid);			    	 
  			    	
  			    	 //给下拉列表赋初始值
  			    	 $("#myUpdateDepartment").val(data.departmentid);
  			    	 
  			    	 $("#myUpdateBed").val(data.bednumber);
  ...
  function updateInpatient(){
		...
		$.ajax({
			type:'post',//提交方式
			url:'${pageContext.request.contextPath}/inpatient/saveInpatient.action',//修改的地址
			data:data,
			dataType:'json',
			success:function(data){
			alert("修改完成")
...
//添加事件
  ...
  function addInpatient() {
    	...
    	$.ajax({
  		type:'post',//提交方式
  		url:'${pageContext.request.contextPath}/inpatient/addInpatient.action',//修改的地址
  		data:data,
  		dataType:'json',
  		success:function(data){
  			alert("添加完成");
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                  <form id="myUpdateForm" class="form-horizontal">
                      <!-- 隐藏框传hospitalizationid的值 -->
                      <input type="hidden" name="hospitalizationid" id="myUpdatehospitalizationid">
            ...
            <div class="modal-footer">
                ...
                <button onclick="updateInpatient()" type="button" class="btn btn-primary">修改</button>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                  <form id="myAddForm" class="form-horizontal">
					    ...
					    <div class="col-sm-7">
					      ...
					      <label id="hospitalizationtime-error" class="error" for="hospitalizationtime" style="margin-top: 4px;color: red;font-size: 15px;"></label>
					    </div>
					  </div>
					    <div class="form-group">
						    <label for="inputPassword3" class="col-sm-2 control-label">床位号:</label>
            ...
            <div class="modal-footer">
                ...
                <button onclick="addInpatient()" type="button" class="btn btn-primary">添加</button>
            </div>
        </div>
    </div>
</div>
